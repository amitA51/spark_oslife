...
const PersonalItemDetailModal: React.FC<PersonalItemDetailModalProps> = ({
  ...
  useEffect(() => {
    if (item) {
      ...
      const fetchRelated = async () => {
        ...
        setRelatedItems([]);
        try {
          const related = await geminiService.findRelatedPersonalItems(item, personalItems);
  ...
  }, [isEditing, editState, item, setHasUnsavedChanges]);
        ...
        {contextItems && (
          <footer className="p-2 border-t border-[var(--border-primary)] sticky bottom-0 bg-[var(--bg-secondary)]/80 backdrop-blur-sm flex justify-between">
            <button
              ...
              disabled={!prevItem}
              className="p-2 rounded-full text-white disabled:text-gray-600"
            >
