...
// Icons
import { CloseIcon, EditIcon, CheckCircleIcon, StarIcon, TrashIcon, SparklesIcon, getFileIcon, ChevronLeftIcon, DownloadIcon } from './icons';
...
const PersonalItemDetailModal: React.FC<PersonalItemDetailModalProps> = ({ item, onClose, onUpdate, onDelete, contextItems }) => {
    const { state, dispatch: appDispatch } = useContext(AppContext);
    const [isClosing, setIsClosing] = useState(false);
    const [isEditing, setIsEditing] = useState(false);
    
    const [editState, dispatch] = useReducer(editReducer, item ? createInitialEditState(item) : createInitialEditState({} as PersonalItem));
    ...
    useEffect(() => {
        if (item) {
            ...
            const fetchRelated = async () => {
                ...
                setRelatedItems([]);
                try {
                    const related = await geminiService.findRelatedPersonalItems(item, state.personalItems);
    ...
    useEffect(() => {
        if (isEditing && item) {
            const initialStateForItem = createInitialEditState(item);
            const isDirty = JSON.stringify(editState) !== JSON.stringify(initialStateForItem);
            if (isDirty) {
                appDispatch({ type: 'SET_UNSAVED_CHANGES' });
            }
        }
    }, [isEditing, editState, item, appDispatch]);
    ...
    const handleClose = (nextItem?: PersonalItem) => {
        ...
        setIsClosing(true);
        setTimeout(() => onClose(nextItem), 300);
    };
    ...
    const renderViewContent = () => {
        if (!item) return null;
        switch(item.type) {
            case 'task': return <TaskView item={item} onUpdate={onUpdate} />;
            case 'book': return <BookView item={item} onUpdate={onUpdate} />;
            case 'workout': return <WorkoutView item={item} onUpdate={onUpdate} />;
            case 'habit': return <HabitView item={item} onUpdate={onUpdate} />;
    ...
    const renderEditContent = () => {
        if (!item) return null;
        switch(item.type) {
            case 'task': return <TaskEdit editState={editState} dispatch={dispatch} />;
            case 'book': return <BookEdit editState={editState} dispatch={dispatch} />;
            case 'workout': return <WorkoutEdit editState={editState} dispatch={dispatch} />;
            case 'habit': return <HabitEdit editState={editState} dispatch={dispatch} />;
                ...
                <div className="space-y-3">
                    {attachments.map((file, index) => {
                        if (file.mimeType.startsWith('image/')) {
                            return (
                                <div key={index} className="rounded-xl overflow-hidden border border-[var(--border-primary)] bg-black/30">
                                    ...
                                    <div className="p-2 bg-black/50 flex justify-between items-center">
                                        ...
                                        <a href={file.url} download={file.name} className="text-[var(--dynamic-accent-highlight)] hover:underline text-xs">הורד</a>
                                    </div>
                                </div>
                            );
                        }
                        if (file.mimeType.startsWith('video/')) {
                         ...
                         if (file.mimeType.startsWith('audio/')) {
                            return (
                                <div key={index} className="rounded-xl border border-[var(--border-primary)] bg-black/30 p-3">
                                    <div className="flex items-center gap-2 mb-2">
                                         <div className="p-1.5 bg-[var(--dynamic-accent-start)]/20 rounded-lg text-[var(--dynamic-accent-start)]">{getFileIcon(file.mimeType)}</div>
                        ...
                        return (
                            <a key={index} href={file.url} target="_blank" rel="noopener noreferrer" download={file.name} className="group flex items-center gap-3 bg-[var(--bg-card)] hover:bg-white/10 p-3 rounded-xl transition-all border border-[var(--border-primary)] hover:border-[var(--dynamic-accent-start)]/50">
                                <div className="p-2 bg-[var(--bg-secondary)] rounded-lg text-gray-300 group-hover:text-[var(--dynamic-accent-highlight)] transition-colors">
                                     {getFileIcon(file.mimeType)}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <span className="text-[var(--text-primary)] font-medium truncate block">{file.name}</span>
                                    <span className="text-xs text-[var(--text-secondary)]">{Math.round(file.size / 1024)} KB</span>
                                </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-end justify-center z-50" onClick={() => handleClose()}>
            ...
            >
                 <header className="p-4 border-b border-[var(--border-primary)] flex justify-between items-center sticky top-0 bg-transparent backdrop-blur-sm z-10 rounded-t-3xl">
                    <div className="flex items-center gap-3 overflow-hidden">
                        <div className="relative">
                           <div className={`h-10 w-10 flex items-center justify-center rounded-lg transition-all ${isEditing ? 'bg-[var(--bg-secondary)] text-[var(--text-secondary)]' : 'text-[var(--accent-highlight)]'}`}>
                                <Icon className="h-6 w-6" />
                           </div>
                           {isEditing && isSuggestingIcon && <SparklesIcon className="w-4 h-4 text-yellow-400 absolute -top-1 -right-1 animate-pulse" />}
                        </div>
                        ...
                        {isEditing ? (
                            ...
                            <h2 id="p-item-title" className="text-xl font-bold text-[var(--text-primary)] truncate">{item.title}</h2>
                        )}
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        {!isEditing && (
                            <button onClick={handleDelete} className="text-[var(--text-secondary)] hover:text-red-400 transition-colors p-2 rounded-full active:scale-95">
                ...
                <div className="p-6 overflow-y-auto flex-grow space-y-6">
                    {isEditing && (
                         <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="spaceId" value={editState.spaceId} onChange={(e) => dispatch({type:'SET_FIELD', payload: {field: 'spaceId', value: e.target.value}})} className={inputStyles}>
                            ...
                            <div>
                                ...
                                <select id="projectId" value={editState.projectId} onChange={(e) => dispatch({type:'SET_FIELD', payload: {field: 'projectId', value: e.target.value}})} className={inputStyles}>
                                    <option value="">ללא</option>
                    ...
                    {isEditing ? (
                        <AttachmentManager attachments={editState.attachments} onAttachmentsChange={(atts) => dispatch({type: 'SET_FIELD', payload: { field: 'attachments', value: atts}})} />
                    ) : (
                        ...
                        <div className="border-t border-[var(--border-primary)] pt-6 mt-6">
                            ...
                            {isLoadingRelated ? (
                                ...
                                <div className="space-y-2">
                                    ...
                                    <button key={related.id} onClick={() => handleClose(related)} className="w-full text-right bg-[var(--bg-card)] hover:bg-white/5 p-3 rounded-xl transition-colors border border-[var(--border-primary)] hover:border-[var(--dynamic-accent-start)]/50 active:scale-95">
                                        <p className="font-semibold text-[var(--text-primary)] truncate">{related.title}</p>
                                        <p className="text-xs text-[var(--text-secondary)] line-clamp-1">{related.content.split('\n')[0]}</p>
                                    </button>
                                    ))}
                                </div>
                            )}
                ...
                {contextItems && (
                     <footer className="p-2 border-t border-[var(--border-primary)] sticky bottom-0 bg-[var(--bg-secondary)]/80 backdrop-blur-sm flex justify-between">
                        <button onClick={() => handleClose(prevItem!)} disabled={!prevItem} className="p-2 rounded-full text-white disabled:text-gray-600">
                           <ChevronLeftIcon className="w-6 h-6 transform rotate-180" />
                        </button>
                        <button onClick={() => handleClose(nextItem!)} disabled={!nextItem} className="p-2 rounded-full text-white disabled:text-gray-600">
