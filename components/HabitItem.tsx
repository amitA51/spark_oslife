...
const HabitItem: React.FC<HabitItemProps> = ({ item, onUpdate, onDelete, onSelect, onContextMenu, index }) => {
  ...
  const cleanDays = isBadHabit && item.lastCompleted 
      ? Math.floor((new Date().getTime() - new Date(item.lastCompleted).getTime()) / (1000 * 3600 * 24))
      : 0;
  ...
  const bgClass = isBadHabit ? 'border-red-500/20 bg-red-900/5' : (isCompletedToday ? 'bg-[var(--dynamic-accent-start)]/20 border-[var(--dynamic-accent-start)]/50 completed-habit' : '');
  const iconColor = isBadHabit ? 'text-red-400' : (item.streak && item.streak > 0 ? 'text-[var(--dynamic-accent-start)] svg-glow' : 'text-gray-600');
  ...
  return (
    ...
    >
      {justCompleted && !isBadHabit && Array.from({ length: 7 }).map((_, i) => (
      ...
      <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4 flex-1 overflow-hidden">
              ...
              <div className="flex-1 overflow-hidden">
                <p className={`text-lg font-semibold ${isBadHabit ? 'text-red-100' : 'text-[var(--text-primary)]'}`}>{item.title}</p>
             ...
             isConfirmingReset ? (
                 <div className="flex flex-col gap-1">
                     ...
                     <div className="flex gap-1">
                        <button onClick={(e) => { e.stopPropagation(); handleRelapse(); }} className="bg-red-500 text-white text-xs px-2 py-1 rounded hover:bg-red-600">כן</button>
                        <button onClick={(e) => { e.stopPropagation(); setIsConfirmingReset(false); }} className="bg-gray-600 text-white text-xs px-2 py-1 rounded hover:bg-gray-500">לא</button>
                     </div>
                 </div>
             ) : (
                <button
                    onClick={(e) => { e.stopPropagation(); setIsConfirmingReset(true); }}
                    className="relative w-10 h-10 flex items-center justify-center rounded-full transition-all transform hover:scale-110 active:scale-95 bg-white/5 hover:bg-red-500/20 text-gray-400 hover:text-red-400"
