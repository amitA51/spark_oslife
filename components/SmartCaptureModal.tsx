...
import { CloseIcon, SparklesIcon, CheckCircleIcon, LightbulbIcon, ClipboardListIcon, FlameIcon } from './icons';
...
const SmartCaptureModal: React.FC<SmartCaptureModalProps> = ({ isOpen, onClose, setActiveScreen, showStatus }) => {
    ...
    useEffect(() => {
        ...
        const analyze = async () => {
            ...
            setError('');
            try {
                const result = await geminiService.smartParseInput(debouncedValue, state.spaces, state.personalItems);
        ...
        analyze();
    }, [debouncedValue, state.spaces, state.personalItems]);
    ...
    const suggestedSpace = aiSuggestion?.suggestedSpaceId
        ? state.spaces.find(s => s.id === aiSuggestion.suggestedSpaceId)
        : null;

    return (
        <DraggableModalWrapper onClose={onClose} className="w-[95vw] max-w-lg bg-[var(--bg-card)] rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-[var(--border-primary)] flex flex-col animate-scale-in">
            ...
            <div className="p-4">
                <textarea
                    ...
                    className="w-full h-32 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-lg p-3 text-lg text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-[var(--dynamic-accent-start)] resize-none"
                    autoFocus
                />
            </div>

            <div className="p-4 min-h-[120px] border-t border-[var(--border-primary)]">
                {isLoading && <div className="flex items-center justify-center gap-2 text-gray-400"><LoadingSpinner className="w-5 h-5" /><p>AI חושב...</p></div>}
                {error && <p className="text-center text-red-400">{error}</p>}
                
                {!isLoading && aiSuggestion && (
                    <div className="space-y-3 animate-item-enter-fi">
                        <p className="text-sm font-semibold text-gray-400">הצעה:</p>
                        <div className="p-3 bg-[var(--bg-secondary)] rounded-lg border border-[var(--border-primary)] space-y-2">
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-sm">
                                ...
                                {aiSuggestion.dueDate && <span className="px-2 py-1 bg-black/30 rounded-full">🗓️ {new Date(aiSuggestion.dueDate).toLocaleDateString('he-IL')}</span>}
                                {suggestedSpace && <span className="px-2 py-1 bg-black/30 rounded-full" style={{ color: suggestedSpace.color }}>● {suggestedSpace.name}</span>}
                            </div>
                        </div>
                    </div>
                )}
            ...
            <div className="p-4 border-t border-[var(--border-primary)] flex gap-2">
                <button onClick={onClose} className="flex-1 py-3 px-4 rounded-xl bg-[var(--bg-secondary)] text-white font-semibold">ביטול</button>
                <button onClick={handleCreate} disabled={!aiSuggestion} className="flex-1 py-3 px-4 rounded-xl bg-[var(--accent-gradient)] text-white font-bold disabled:opacity-50 disabled:cursor-not-allowed">
                    צור
                </button>
            </div>
