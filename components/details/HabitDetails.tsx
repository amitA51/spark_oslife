...
export const HabitView: React.FC<ViewProps> = ({ item }) => {
    return (
        <div className="space-y-4">
             <div className={`p-4 rounded-xl border ${item.habitType === 'bad' ? 'bg-red-500/10 border-red-500/30' : 'bg-green-500/10 border-green-500/30'} flex items-center gap-3`}>
                {item.habitType === 'bad' ? (
                    <>
                        <div className="p-2 bg-red-500/20 rounded-lg"><ShieldCheckIcon className="w-6 h-6 text-red-400"/></div>
                        <div>
                    ...
                    <>
                        <div className="p-2 bg-green-500/20 rounded-lg"><FlameIcon className="w-6 h-6 text-green-400"/></div>
                        <div>
            ...
            {item.content && (
                <div>
                    <h4 className="text-sm font-semibold text-[var(--accent-highlight)] mb-2 uppercase tracking-wider">הערות</h4>
                    <MarkdownRenderer content={item.content} />
                </div>
            )}
            <div>
                 <h4 className="text-sm font-semibold text-[var(--accent-highlight)] mb-2 uppercase tracking-wider">תזכורת</h4>
                 <p className="text-sm text-gray-300">
                    {item.reminderEnabled ? `תזכורת יומית מופעלת לשעה ${item.reminderTime}.` : "אין תזכורות מופעלות להרגל זה."}
                 </p>
            </div>
        </div>
    );
...
export const HabitEdit: React.FC<EditProps> = ({ editState, dispatch }) => {
    return (
        <div className="space-y-6">
            <div>
                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-2">סוג הרגל</label>
                <div className="flex p-1 bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-primary)]">
                    <button
                        type="button"
                        onClick={() => dispatch({type: 'SET_FIELD', payload: { field: 'habitType', value: 'good' }})}
                    ...
                    <button
                        type="button"
                        onClick={() => dispatch({type: 'SET_FIELD', payload: { field: 'habitType', value: 'bad' }})}
            ...
            <div className="p-4 bg-[var(--bg-secondary)] rounded-lg">
                <div className="flex justify-between items-center">
                    <label htmlFor="reminderEnabled" className="font-medium text-white">תזכורת יומית</label>
                    <ToggleSwitch 
                        id="reminderEnabled"
                        checked={editState.reminderEnabled || false}
                        onChange={val => dispatch({type: 'SET_FIELD', payload: { field: 'reminderEnabled', value: val }})}
                    />
                </div>
                {editState.reminderEnabled && (
                    <div className="mt-3">
                        <label htmlFor="reminderTime" className="block text-sm font-medium text-[var(--text-secondary)] mb-1">שעת התזכורת</label>
                        <input
                            type="time"
                            id="reminderTime"
                            value={editState.reminderTime || '09:00'}
                            onChange={e => dispatch({type: 'SET_FIELD', payload: { field: 'reminderTime', value: e.target.value }})}
