...
export const TaskView: React.FC<ViewProps> = ({ item, onUpdate }) => {
    ...
    return (
        <div>
            <h4 className="text-sm font-semibold text-[var(--accent-highlight)] mb-2 uppercase tracking-wider">תת-משימות</h4>
            <div className="space-y-2">
                {item.subTasks?.map((st, index) => (
                    ...
                    >
                        <DragHandleIcon className="w-5 h-5 text-gray-600 cursor-grab" />
                        <input type="checkbox" checked={st.isCompleted} onChange={() => handleToggleSubTask(st.id)} className="h-5 w-5 rounded bg-black/30 border-gray-600 text-[var(--dynamic-accent-start)] focus:ring-[var(--dynamic-accent-start)] cursor-pointer" />
                        <span className={`flex-1 ${st.isCompleted ? 'line-through text-gray-500' : 'text-gray-200'}`}>{st.title}</span>
                        <button onClick={() => handleRemoveSubTask(st.id)} className="opacity-0 group-hover:opacity-100 text-gray-500 hover:text-red-400"><TrashIcon className="w-4 h-4" /></button>
                    </div>
                ))}
                <div className="flex items-center gap-2">
                    <input type="text" value={newSubTask} onChange={e => setNewSubTask(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleAddSubTask()} placeholder="הוסף תת-משימה..." className="flex-1 text-sm bg-transparent border-b border-[var(--border-primary)] focus:border-[var(--dynamic-accent-start)] focus:outline-none py-1" />
                    <button onClick={handleAddSubTask} className="text-[var(--dynamic-accent-highlight)] p-1"><AddIcon className="w-5 h-5" /></button>
                </div>
            </div>


            <div className="mt-6 pt-4 border-t border-[var(--border-primary)]">
                <TimeBlocker task={item} />
            </div>
        </div >
    );
};
...
export const TaskEdit: React.FC<EditProps> = ({ editState, dispatch }) => (
    <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
            ...
            <div>
                <label htmlFor="dueTime" className="block text-sm font-medium text-[var(--text-secondary)] mb-1">שעת יעד (תזכורת)</label>
                <input
                    type="time"
                    id="dueTime"
                    value={editState.dueTime}
                    onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'dueTime', value: e.target.value } })}
                    className={inputStyles}
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            <div>
                <label htmlFor="autoDeleteAfter" className="block text-sm font-medium text-[var(--text-secondary)] mb-1">מחיקה אוטומטית</label>
                <select
                    id="autoDeleteAfter"
