...
const QuickNoteModal: React.FC<QuickNoteModalProps> = ({ date, onClose }) => {
    ...
    return (
        <DraggableModalWrapper onClose={onClose} className="w-[95vw] max-w-md bg-[var(--bg-card)] rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.6)] border border-[var(--border-primary)] overflow-hidden animate-scale-in">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
                <div>
                    <input 
                        ...
                        className="w-full bg-transparent text-lg font-bold text-white placeholder:text-gray-500 focus:outline-none border-b border-[var(--border-primary)] focus:border-[var(--dynamic-accent-start)] pb-2 transition-colors"
                        required
                    />
                </div>
                ...
                <div>
                    <textarea 
                        ...
                        rows={4}
                        className="w-full bg-[var(--bg-secondary)]/50 rounded-xl p-3 text-sm text-[var(--text-primary)] placeholder:text-gray-600 focus:outline-none resize-none"
                    />
                </div>
