...
import { ShieldExclamationIcon, SparklesIcon, CheckCircleIcon, CloseIcon, StopwatchIcon } from './icons';
...
const ComfortZoneWidget: React.FC = () => {
    ...
    if (!challenge) {
        return (
            <button 
                ...
                disabled={isLoading}
                className="w-full themed-card p-6 flex flex-col items-center justify-center gap-3 border-2 border-dashed border-[var(--border-primary)] hover:border-[var(--dynamic-accent-start)] hover:bg-white/5 transition-all group"
            >
                {isLoading ? <LoadingSpinner /> : (
                    <>
                        ...
                        <div className="text-center">
                            <h3 className="font-bold text-white">שובר הנוחות</h3>
    ...
    if (challenge.status === 'hidden') {
        return (
            <div 
                className={`relative themed-card overflow-hidden p-8 text-center select-none cursor-pointer transition-all duration-500 ${isHolding ? 'scale-[0.98] ring-4 ring-[var(--dynamic-accent-start)]/50' : ''}`}
            ...
            >
                ...
                <div className={`absolute inset-0 opacity-20 pointer-events-none transition-opacity duration-300 ${isHolding ? 'opacity-40' : ''}`}
                     style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.8' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E")`}} 
                />
                
                <div className="relative z-10 flex flex-col items-center gap-4">
                    <div className={`p-4 rounded-full bg-white/5 border border-white/10 transition-all duration-1000 ${isHolding ? 'animate-pulse shadow-[0_0_30px_var(--dynamic-accent-glow)]' : ''}`}>
                    ...
                    <p className="text-sm text-gray-400">{isHolding ? "חושף..." : "לחץ ארוך לחשיפה"}</p>
                </div>

                 {/* Progress ring for hold */}
                 {isHolding && (
                    <svg className="absolute inset-0 w-full h-full pointer-events-none">
                        <rect width="100%" height="100%" fill="none" stroke="var(--dynamic-accent-start)" strokeWidth="4" 
    ...
    if (challenge.status === 'active') {
        return (
            <div className="themed-card p-5 border-l-4 border-l-[var(--dynamic-accent-start)] relative overflow-hidden animate-flip-in">
                ...
                <p className="text-lg font-bold text-white mb-6 leading-snug">
                    {challenge.text}
                </p>
    ...
    return (
        <div className={`themed-card p-5 flex items-center gap-4 ${challenge.status === 'completed' ? 'bg-green-900/10 border-green-500/30' : 'bg-red-900/10 border-red-500/30 opacity-60'}`}>
            <div className={`w-12 h-12 rounded-full flex items-center justify-center ${challenge.status === 'completed' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                {challenge.status === 'completed' ? <CheckCircleIcon className="w-6 h-6" /> : <CloseIcon className="w-6 h-6" />}
            </div>
            <div>
                <h3 className="font-bold text-white">{challenge.status === 'completed' ? 'אתגר הושלם!' : 'אתגר נכשל'}</h3>
