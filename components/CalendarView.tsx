    ...
    <div className="flex justify-between items-center mb-4 px-2">
      <div className="flex items-center gap-2">
        <button
          ...
          className="text-sm font-semibold bg-[var(--bg-secondary)] px-4 py-2 rounded-full hover:bg-white/10 transition-colors"
        >
          היום
        </button>
        <button onClick={() => changeDate(-1)} className="p-2 rounded-full hover:bg-white/10">
      ...
      <div className="flex items-center gap-1 p-1 bg-[var(--bg-secondary)] rounded-full text-sm">
        <button
          ...
          className={`px-4 py-1.5 rounded-full ${viewMode === 'month' ? 'bg-white/20 text-white' : 'text-[var(--text-secondary)]'}`}
        >
          חודש
        </button>
        <button
          onClick={() => setViewMode('week')}
          className={`px-4 py-1.5 rounded-full ${viewMode === 'week' ? 'bg-white/20 text-white' : 'text-[var(--text-secondary)]'}`}
        >
          שבוע
        </button>
      </div>
...
const CalendarItem: React.FC<{ item: PersonalItem; onSelect: (e: React.MouseEvent) => void }> = ({
  item,
  onSelect,
}) => {
...
const CalendarView: React.FC<CalendarViewProps> = ({
  ...
  const combinedItemsByDate = useMemo(() => {
    ...
    // Add Personal Items
    items.forEach(item => {
      const dateStr = item.dueDate || (item.metadata as any)?.targetDate;
    ...
    try {
      ...
      if (item && item.id) {
        ...
        if (item.metadata && 'targetDate' in item.metadata) {
          ...
          updates.metadata = { targetDate: newDate } as any;
        }
    ...
    return (
      <div className="grid grid-cols-7 border-t border-r border-[var(--border-primary)]">
        {['א', 'ב', 'ג', 'ד', 'ה', 'ו', 'ש'].map(d => (
          <div
            key={d}
            className="text-center font-semibold text-sm text-[var(--text-secondary)] py-2 border-b border-l border-[var(--border-primary)] bg-[var(--bg-secondary)]"
          >
            {d}
          </div>
        ))}
        {Array.from({ length: firstDayOfMonth }).map((_, i) => (
          <div key={`empty-${i}`} className="border-b border-l border-[var(--border-primary)]" />
        ))}
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          return (
            ...
            >
              <span
                className={`text-sm mb-1.5 block ${isToday ? 'font-bold text-white bg-[var(--dynamic-accent-start)] rounded-full w-6 h-6 flex items-center justify-center' : 'text-[var(--text-secondary)]'}`}
              >
                {day}
              </span>
          ...
          return (
            ...
            >
              <div className="text-center mb-2">
                <p className={`text-xs ${isToday ? 'text-white' : 'text-[var(--text-secondary)]'}`}>
                  {['א', 'ב', 'ג', 'ד', 'ה', 'ו', 'ש'][day.getDay()]}
                </p>
                <p
                  className={`font-bold text-lg ${isToday ? 'text-white' : 'text-[var(--text-secondary)]'}`}
                >
                  {day.getDate()}
                </p>
              </div>
