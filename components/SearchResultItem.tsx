...
import type { UniversalSearchResult, GoogleCalendarEvent } from '../types';
import {
  CheckCircleIcon,
  ClipboardListIcon,
  LinkIcon,
    ...
    <>
      {parts.map((part, i) =>
        part.toLowerCase() === query.toLowerCase() ? (
          <mark
            key={i}
            className="bg-[var(--dynamic-accent-highlight)]/50 text-white px-0 rounded-sm"
          >
            {part}
          </mark>
        ) : (
...
const getIconForType = (type: UniversalSearchResult['type']) => {
  const className = 'w-5 h-5 text-muted';
  switch (type) {
    case 'task':
    ...
    case 'roadmap':
      return <RoadmapIcon className={className} />;
    case 'learning':
      return <SummarizeIcon className={className} />;
    case 'calendar':
...
const SearchResultItem: React.FC<SearchResultItemProps> = ({ result, query, onSelectItem }) => {
  ...
  return (
    <button
      onClick={handleClick}
      className="w-full text-right themed-card p-4 hover:border-[var(--dynamic-accent-start)]/50 transition-all"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 overflow-hidden">
          <div className="flex justify-between items-baseline">
            ...
            <span className="text-xs text-muted font-mono shrink-0">{formattedDate}</span>
          </div>
          {result.content && (
