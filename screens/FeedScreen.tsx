...
const FilterButton: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`px-4 py-2 text-sm rounded-full transition-all shrink-0 transform hover:scale-105 active:scale-95 font-bold ${isActive
          ...
          <>
            ...
            {!isLoading && filteredItems.length === 0 &&
              <EmptyState
                ...
                description={feedItems.length > 0 ? "נסה לבחור סינון אחר או לרענן את הפידים שלך." : "לחץ על כפתור הרענון כדי למשוך תוכן חדש מהמקורות שהגדרת."}
                action={{
                  label: "רענן פידים",
                  onClick: () => handleRefresh(false),
