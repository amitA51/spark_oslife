...
const AssistantScreen: React.FC<AssistantScreenProps> = ({ setActiveScreen }) => {
  ...
  useEffect(() => {
    const initChat = async () => {
      try {
        const chatSession = await createAssistantChat(state.feedItems, state.personalItems);
        setChat(chatSession);
        ...
        setMessages([{id: 'error', role: 'model', text: 'שגיאה בהפעלת היועץ. נסה שוב מאוחר יותר.', isTyping: false}])
      } finally {
        setIsLoading(false);
      }
    };
    initChat();
  }, [state.feedItems, state.personalItems]);
  ...
  const handleSend = async () => {
    ...
    playClick();
    const userMessage: Message = { id: Date.now().toString(), role: 'user', text: inputValue, isTyping: false };
    ...
    try {
        ...
        setMessages(prev => [...prev, { id: modelMessageId, role: 'model', text: '', isTyping: true }]);
        ...
        setMessages(prev => prev.map(msg => 
             msg.id === modelMessageId ? { ...msg, isTyping: false } : msg
        ));

    } catch (error) {
      console.error("Failed to get assistant response:", error);
      const errorMessage: Message = { id: `error-${Date.now()}`, role: 'model', text: 'התנצלותי, נתקלתי בשגיאה.', isTyping: false};
      ...
      <header className="flex justify-between items-center mb-4 px-4 sticky top-0 bg-[var(--bg-primary)]/80 backdrop-blur-md py-3 z-20 border-b border-[var(--border-primary)] -mx-4">
        <div className="flex items-center gap-2">
            ...
            <div>
                ...
                <p className="text-sm text-[var(--text-secondary)] mt-1">שאל שאלות על המידע האישי שלך</p>
            </div>
        </div>
      </header>
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-xl p-4 rounded-3xl shadow-md ${msg.role === 'user' ? 'bg-[var(--bg-card)] text-white rounded-br-lg' : 'bg-[var(--bg-secondary)] text-gray-200 rounded-bl-lg'}`}>
               ...
               <MarkdownRenderer 
                  content={msg.text} 
                  animate={msg.role === 'model' && (msg.isTyping || msg.id === messages[messages.length - 1].id)} 
               />
            </div>
          </div>
        ))}
