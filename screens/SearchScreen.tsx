...
import type { Screen, PersonalItem, FeedItem, UniversalSearchResult, SearchFilters, GoogleCalendarEvent } from '../types';
...
export default function SearchScreen({ setActiveScreen }: SearchScreenProps) {
    ...
    const { personalItems, feedItems, calendarEvents, settings } = state;
    ...
    const allSearchableData = useMemo((): UniversalSearchResult[] => {
        ...
        return [...personal, ...feed, ...calendar].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    ...
    const displayedResults = useMemo(() => {
        const source = isAiSearch ? allSearchableData.filter(item => aiSourceIds.has(item.id)) : localResults;
        ...
        <div className="flex h-[calc(100vh-80px)]">
            ...
            <div className="flex-1 pt-4 pb-4 px-4 flex flex-col">
                <header className="mb-6 sticky top-0 bg-[var(--bg-primary)]/80 backdrop-blur-md py-3 z-20 border-b border-[var(--border-primary)] -mx-4 px-4">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex items-center gap-2">
                             <button onClick={() => setIsFilterPanelOpen(true)} className="p-2 rounded-full text-[var(--text-secondary)] hover:bg-[var(--bg-secondary)] hover:text-white transition-colors">
                    ...
                    <div className="relative flex items-center gap-2">
                        <div className="relative flex-grow">
                            ...
                            <input
                                type="search" value={query} onChange={(e) => handleQueryChange(e.target.value)}
                        ...
                        <button onClick={handleAiSearch} disabled={isAiLoading || query.length < 2} className="p-3 bg-[var(--accent-gradient)] text-white rounded-2xl transition-all shrink-0 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 active:scale-95 hover:shadow-[0_0_15px_var(--dynamic-accent-glow)]">
                ...
                <div className="flex-1 overflow-y-auto space-y-6">
                    {isAiLoading && <div className="text-center py-8"><LoadingSpinner /></div>}
                    ...
                    {Object.entries(resultsByCategory).map(([category, items]: [string, UniversalSearchResult[]]) => (
                        <section key={category}>
                             <h2 className="text-sm font-semibold text-[var(--text-secondary)] uppercase tracking-wider mb-3 px-1">{categoryLabels[category] || category}</h2>
                    ...
                    {!query && !isAiLoading && (
                        <div className="text-center text-gray-500 mt-16 flex flex-col items-center">
                            <SearchIcon className="w-16 h-16 text-gray-700 mb-4"/>
                            <h2 className="text-lg font-semibold text-white">חיפוש בכל הידע שלך</h2>
